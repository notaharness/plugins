...
is_player_session "$target" || { echo "kill.sh: $target is not a player session (its tags do not say $TAG_SPAWNER + $TAG_SESSION_TYPE $SESSION_TYPE_WORKTREE or $SESSION_TYPE_DIR); nothing killed" >&2; exit 1; }
